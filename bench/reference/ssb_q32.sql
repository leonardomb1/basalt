-- DuckDB reference for bench/ssb/q32.sql.
COPY (
...
JOIN (SELECT c_custkey, c_city FROM read_parquet('@DATA@/ssb/customer.parquet') WHERE c_nation = 'UNITED STATES') c ON lo_custkey = c_custkey
