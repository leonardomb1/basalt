-- TPC-DS q52 — brand revenue, one manager
--
...
WITH dt AS (SELECT d_date_sk, d_year FROM IDENTIFIER($data || '/tpcds/date_dim.parquet') WHERE d_moy = 11 AND d_year = 2000),
     it AS (SELECT i_item_sk, i_brand_id, i_brand FROM IDENTIFIER($data || '/tpcds/item.parquet') WHERE i_manager_id = 1)
SELECT dt.d_year, it.i_brand_id AS brand_id, it.i_brand AS brand, SUM(ss_ext_sales_price) AS ext_price
