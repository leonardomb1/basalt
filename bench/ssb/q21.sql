-- SSB Q2.1 fact x date x supplier x part, group by year and brand
-- Dimensions are lifted into CTEs: basalt requires the right side of a join
-- to be a CTE. The reference in bench/reference/ssb_q21.sql mirrors them inline.
PARAM data STRING;
PARAM out  STRING;
...
WITH d AS (SELECT d_datekey, d_year FROM IDENTIFIER($data || '/ssb/date.parquet')),
     ...
     p AS (SELECT p_partkey, p_brand1 FROM IDENTIFIER($data || '/ssb/part.parquet') WHERE p_category = 'MFGR#12')
