-- SSB Q4.2 profit by year, supplier nation and category
-- Dimensions are lifted into CTEs: basalt requires the right side of a join
-- to be a CTE. The reference in bench/reference/ssb_q42.sql mirrors them inline.
PARAM data STRING;
PARAM out  STRING;
...
WITH d AS (SELECT d_datekey, d_year FROM IDENTIFIER($data || '/ssb/date.parquet') WHERE d_year IN (1997, 1998)),
     ...
     s AS (SELECT s_suppkey, s_nation FROM IDENTIFIER($data || '/ssb/supplier.parquet') WHERE s_region = 'AMERICA'),
     p AS (SELECT p_partkey, p_category FROM IDENTIFIER($data || '/ssb/part.parquet') WHERE p_mfgr IN ('MFGR#1', 'MFGR#2'))
