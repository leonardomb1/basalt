-- SSB Q3.2 city pairs within one nation
-- Dimensions are lifted into CTEs: basalt requires the right side of a join
-- to be a CTE. The reference in bench/reference/ssb_q32.sql mirrors them inline.
PARAM data STRING;
PARAM out  STRING;
...
WITH d AS (SELECT d_datekey, d_year FROM IDENTIFIER($data || '/ssb/date.parquet') WHERE d_year >= 1992 AND d_year <= 1997),
     c AS (SELECT c_custkey, c_city FROM IDENTIFIER($data || '/ssb/customer.parquet') WHERE c_nation = 'UNITED STATES'),
