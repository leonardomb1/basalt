                        ...
                        const app_cipher = app_cipher: switch (handshake_cipher) {
                            inline else => |*p, tag| switch (tls_version) {
                                .tls_1_3 => {
                                    ...
                                    const fin_part = .{@intFromEnum(tls.HandshakeType.finished)} ++
                                        array(u24, u8, verify_data);

                                    var clear_buf: [empty_cert_msg.len + fin_part.len + 1]u8 = undefined;
                                    var clear_len: usize = 0;
                                    if (client_cert_requested) {
                                        @memcpy(clear_buf[0..empty_cert_msg.len], &empty_cert_msg);
                                        clear_len = empty_cert_msg.len;
                                    }
                                    @memcpy(clear_buf[clear_len..][0..fin_part.len], &fin_part);
                                    clear_len += fin_part.len;
                                    clear_buf[clear_len] = @intFromEnum(tls.ContentType.handshake);
                                    clear_len += 1;
                                    const out_cleartext = clear_buf[0..clear_len];

                                    var finished_msg: [tls.record_header_len + clear_buf.len + P.AEAD.tag_length]u8 = undefined;
                                    const wrapped_len = clear_len + P.AEAD.tag_length;
                                    finished_msg[0] = @intFromEnum(tls.ContentType.application_data);
                                    mem.writeInt(u16, finished_msg[1..3], @intFromEnum(tls.ProtocolVersion.tls_1_2), .big);
                                    mem.writeInt(u16, finished_msg[3..5], @intCast(wrapped_len), .big);

                                    const ad = finished_msg[0..tls.record_header_len];
                                    const ciphertext = finished_msg[tls.record_header_len..][0..clear_len];
                                    const auth_tag = finished_msg[tls.record_header_len + clear_len ..][0..P.AEAD.tag_length];
                                    ...
                                    var all_msgs_vec: [2][]const u8 = .{
                                        &client_change_cipher_spec_msg,
                                        finished_msg[0 .. tls.record_header_len + wrapped_len],
                                    };
                                    try output.writeVecAll(&all_msgs_vec);
                                    try output.flush();
