...
test "request source: non-array/object bodies are rejected by name" {
    const gpa = std.testing.allocator;
    try std.testing.expectError(error.ExpectedJsonArrayOrObject, RequestSource.open(gpa, "42"));
    ...
    try std.testing.expectError(error.SyntaxError, RequestSource.open(gpa, "{nope"));
}
