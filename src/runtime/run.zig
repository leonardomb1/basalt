...
test "csv aggregate: min/max on inferred numeric columns compare numerically, not lexically" {
    ...
    const got = try runToString(alloc, &tmp, "id\n9\n10\n2\n", "  | aggregate mn = min(id), mx = max(id), s = sum(id)");
...
test "join: an empty build side drops all rows (inner) and null-fills (left)" {
    ...
    const script = try std.fmt.allocPrint(alloc,
        "@batch\nlet labels = read csv \"{s}/lookup.csv\"\n" ++
            "read csv \"{s}/in.csv\"\n  | join inner labels on code = code\n  | select id, label\n  | write csv \"{s}/inner.csv\"\n" ++
            "read csv \"{s}/in.csv\"\n  | join left labels on code = code\n  | select id, label\n  | write csv \"{s}/left.csv\"",
...
test "join: duplicate build keys fan out (inner); semi/anti reduce to existence" {
    ...
    const script = try std.fmt.allocPrint(alloc,
        "@batch\nlet labels = read csv \"{s}/lookup.csv\"\n" ++
            "read csv \"{s}/in.csv\"\n  | join inner labels on code = code\n  | select id, label\n  | write csv \"{s}/inner.csv\"\n" ++
            "read csv \"{s}/in.csv\"\n  | join semi labels on code = code\n  | write csv \"{s}/semi.csv\"\n" ++
            "read csv \"{s}/in.csv\"\n  | join anti labels on code = code\n  | write csv \"{s}/anti.csv\"",
...
test "statement-level match dispatches on a resolved param (default arm otherwise)" {
    const alloc = std.testing.allocator;
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
    ...
    const script = try std.fmt.allocPrint(alloc,
        "@batch\nparam mode string = \"small\"\nmatch mode\n" ++
            "  \"big\" => {{ read csv \"{s}/in.csv\" | select id, v | write csv \"{s}/out.csv\" }}\n" ++
            "  _ => {{ read csv \"{s}/in.csv\" | select id | write csv \"{s}/out.csv\" }}\nend",
        .{ base, base, base, base });
    defer alloc.free(script);

    // default value → the `_` arm runs
...
test "for-each on_error=continue with an OutcomeSink: run succeeds, failure recorded" {
    ...
    const script = try std.fmt.allocPrint(alloc,
        "@batch\nfor name in csv \"{s}/names.csv\" @[mode = sequential, on_error = continue]\n  read csv \"{s}/${{name}}.csv\"\n    | write csv \"{s}/out_${{name}}.csv\"",
        .{ base, base, base });
    defer alloc.free(script);
