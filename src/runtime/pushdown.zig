...
test "translatePred: NOT over an OR with a bool literal" {
    ...
    try testing.expectEqualStrings("(NOT (((`a` > 1) OR (1=0))))", (try translatePred(a, not_e, .mysql, testSchema())).?);
}

test "translatePred: `is empty` (null-or-'') is not pushed, only plain `is null` is" {
    var ar = std.heap.ArenaAllocator.init(testing.allocator);
    defer ar.deinit();
    const a = ar.allocator();
    const e = try a.create(ast.Expr);
    e.* = .{ .is_null = .{ .e = try fld(a, "b"), .negated = false, .kind = .is_empty } };
    try testing.expect((try translatePred(a, e, .mysql, testSchema())) == null);
    const n = try a.create(ast.Expr);
    n.* = .{ .is_null = .{ .e = try fld(a, "b"), .negated = false, .kind = .is_null } };
    try testing.expectEqualStrings("(`b` IS NULL)", (try translatePred(a, n, .mysql, testSchema())).?);
}
...
test "planAgg: an untranslatable filter is not pushed but its columns stay projected" {
    ...
    defer ar.deinit();
    const a = ar.allocator();
    // filter lower(b) == 'x' can't become SQL, but the kept filter op still reads
    // `b` — so the projection must fetch it (the pushed set is a superset).
    const args = try a.alloc(*ast.Expr, 1);
    args[0] = try fld(a, "b");
    const call = try a.create(ast.Expr);
    call.* = .{ .call = .{ .name = "lower", .args = args } };
    const lit = try a.create(ast.Expr);
    lit.* = .{ .str_lit = "x" };
    const filt = ast.Stage{ .node = .{ .filter = try bin(a, .eq, call, lit) }, .hints = &.{}, .pos = .{ .line = 0, .col = 0 } };
    const by = try a.alloc(ast.QualName, 1);
    by[0] = .{ .parts = &.{"a"} };
    const aggs = try a.alloc(ast.AggItem, 1);
    aggs[0] = .{ .name = "total", .func = .sum, .arg = try fld(a, "c") };
